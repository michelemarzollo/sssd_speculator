This is a slightly modified version of REST, taken from https://github.com/FasterDecoding/REST/tree/main . The modification is made to support models with a vocabulary size of more than 65536 tokens. The modification is done following [this section](#other-models-and-datastore).
Note that the modification is only done to the Reader, and datastore, if the model has a large vocabulary, should be created with the SSSD method. The construction is completely equivalent, but SSSD adds a flag to signal that the vocabulary is larger than  65536 tokens.
